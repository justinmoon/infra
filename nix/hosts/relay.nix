# Parameterized MoQ relay host.
#
# Usage in flake.nix:
#   import ./relay.nix { hostname = "relay-ash"; domain = "us-east.moq.logos.surf"; }
#
# Each relay runs:
#   - Caddy (TCP 80/443) for ACME cert issuance + health/discover/web endpoints
#   - moq-relay (UDP 443) for QUIC pub/sub
#   - cert-sync copies Caddy-managed certs to the relay on startup
...
let
  ...
  syncCertScript = pkgs.writeShellScript "moq-relay-sync-cert" ''
    ...
    # Wait up to 5 minutes for Caddy to issue the certificate.
  ...
  # Discovery endpoint: static JSON listing all relay endpoints.
  discoverJson = builtins.toJSON {
    relays = [
      { host = "us-east.moq.logos.surf"; region = "us-east"; location = "Ashburn, VA"; }
      { host = "us-west.moq.logos.surf"; region = "us-west"; location = "Hillsboro, OR"; }
      { host = "germany.moq.logos.surf"; region = "eu-central"; location = "Falkenstein, DE"; }
      { host = "singapore.moq.logos.surf"; region = "ap-southeast"; location = "Singapore"; }
    ];
  };
in
{
  ...
  # MoQ overlay (provides pkgs.moq-relay)
  nixpkgs.overlays = [ moq.overlays.default ];

  # ── Nix settings ──────────────────────────────────────────────────────
  # Use kixelated cachix for pre-built moq binaries (avoids compiling Rust from source).
  nix.settings = {
  ...
  # Increase UDP buffer sizes for QUIC. Quinn (the Rust QUIC library) warns
  # at startup if these are too small. Defaults are ~200KB which causes
  ...
  # ── SSH ───────────────────────────────────────────────────────────────
  # Public SSH (key-only) for relay management.
  # We can tighten to Tailscale-only later once all relays are on the mesh.
  services.openssh.openFirewall = lib.mkForce true;

  # ── Caddy (TLS cert management + health/web endpoints) ───────────────
  services.caddy = {
    enable = true;
    ...
    # Disable HTTP/3 so Caddy doesn't claim UDP :443 (MoQ relay needs it).
    globalConfig = ''
      servers {
        protocols h1 h2
      }
  ...
  # Override the NixOS module's systemd unit for cert-sync + web listener.
  systemd.services.moq-relay = {
    after = [ "caddy.service" ];
    requires = [ "caddy.service" ];

    # Add the web HTTP listener (Caddy proxies to this).
    ...
    serviceConfig = {
      ...
      # The module already sets AmbientCapabilities for port < 1024,
      # but we also need CapabilityBoundingSet for the sandbox to allow it.
  ...
  # ── Firewall ──────────────────────────────────────────────────────────
  networking.firewall = {
    allowedTCPPorts = [ 80 443 ];    # Caddy (ACME + HTTPS)
    allowedUDPPorts = [ 443 ];        # MoQ QUIC
  };
  ...
  # ── Disko disk layout (Hetzner Cloud VPS) ────────────────────────────
  disko.devices = {
    disk.main = {
      type = "disk";
      device = "/dev/sda";
